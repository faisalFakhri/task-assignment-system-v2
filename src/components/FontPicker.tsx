    ...
    return (
      <div className="flex items-center gap-1.5">
        ...
        <select
          ...
          className="glass-subtle rounded-full px-2.5 py-1 text-[11px] font-mono text-slate-700 border border-slate-300 outline-none focus:border-slate-400 max-w-[160px]"
          title="Select font — saved in your browser"
        >
          <option value="">Default (Inter)</option>
          {fonts.map((f) => (
            <option key={f.value} value={f.value} style={{ fontFamily: f.value }} className="text-slate-800">
  ...
  return (
    <div className="space-y-3">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="text-xs font-semibold text-slate-200 font-mono">FONT_PREFERENCES</div>
        <span className="text-[11px] font-mono text-slate-400">Per-browser · localStorage</span>
      </div>

      <div className="grid gap-3">
        <label className="block">
          <div className="text-[11px] font-mono text-slate-400 mb-1.5">Select Font</div>
          <select
            value={selected}
            onChange={(e) => onChange(e.target.value)}
            className="w-full glass-subtle rounded-xl px-3 py-2.5 text-xs font-mono text-slate-300 border border-slate-600 outline-none focus:border-violet-400/30"
          >
        ...
        <div className="glass-subtle rounded-xl p-3 border border-slate-600">
          ...
          <div className="text-xs leading-5 text-slate-400 mt-1" style={{ fontFamily: current ? current.value : undefined }}>
            Task Assignment System (Varian C · dark glass)
          </div>
        </div>
        ...
        <div className="flex gap-2">
          <button
            onClick={() => onChange('')}
            className="glass-subtle rounded-full px-3 py-1.5 text-xs font-mono text-slate-400 border border-slate-600 hover:text-slate-200"
          >
            Reset ke Default
          </button>
        </div>
          ...
          <button
            ...
            className={`text-left rounded-xl px-3 py-2 border transition-colors ${
              selected === f.value
                ...
                : 'glass-subtle border-slate-600 text-slate-400 hover:bg-white/5 hover:text-slate-200'
            }`}
          >
